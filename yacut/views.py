...
from . import app, db
from .forms import URLMapForm, UploadForm
...
def short_url_view():
    ...
    try:
        return render_template(
            'index.html', form=form, short=URLMap.create(
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            try:
                ...
                uploaded_files_url = await async_upload_files_to_yadisk(form.files.data)

                # Проверяем, что загрузка прошла успешно и есть URL
                if uploaded_files_url is None or not uploaded_files_url:
                    flash('Не удалось загрузить файлы на Яндекс.Диск.', 'error')
                ...
                for file, url in zip(form.files.data, uploaded_files_url):
                    try:
                        ...
                        flash(f'Ошибка создания короткой ссылки для {file.filename}: {str(e)}', 'error')
                        return render_template('download_files.html', form=form)
                ...
                flash(f'Произошла ошибка при загрузке файлов: {str(e)}', 'error')
                return render_template('download_files.html', form=form)
        else:
