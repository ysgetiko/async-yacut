...
async def async_upload_files_to_yadisk(files: Optional[List] = None) -> List[str]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            raise RuntimeError(f"Ошибка при выполнении загрузки файлов: {e}") from e
...
async def _get_upload_url(session: aiohttp.ClientSession, filename: str) -> str:
...
async def _get_download_url(session: aiohttp.ClientSession, file_path: str) -> str:
